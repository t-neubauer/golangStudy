...
Type can be parameterized with a type parameter.
Useful for implementing generic data structure.

Example: singly-linked list holding any type of value
```golang
...
`go f(x,y,z)` starts a new goroutine funning `f(x,y,z)`

Evaluation of x,y,z happens in current goroutine and execution of f happens in new goroutine.

Goroutines run in same address space, **access to shared memory must be synchronized**. 
...
Create a channel before using it:
`ch := make(chan int)`

By default: Sends and receives block until the other side is ready. 
**This allows go to synchronize without explicit locks or condition variables** 
...
Channels can be buffered. Provide buffer length as second argument to `make` to initialize a buffered channel.

`ch := make(chan int, 100)`

Sends to a buffered channel block only when the buffer is full. Receives block when the buffer is empty.

If buffer get overfilled, it causes a deadlock. 

### Range and Close

A sender can close a channel to indicate that no more values will be sent. 
Receivers can test wether a channel has been closed by assigning a second parameter to the receive expression: `after`

`v, ok := <-ch`

`ok` is false if there are no more values to receive and the channel is closed.

Loop `for i := range c` receives values from the channel repeatedly until it is closed. 

Only sender should close a channel. NEVER the receiver. Sending on a closed channel will cause a panic. 

Another note: 
Channels are not like files. Usually you dont need to close them. Closing only necessary when the receiver must be told there are no more values coming, such as to terminate a `range` loop.
